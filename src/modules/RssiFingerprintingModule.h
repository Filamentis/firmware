...
public:
    ...
    void addSample(const std::string& id, int rssi, double lat, double lon);
    ...
    void clearDatabase();
    // KNN localization: returns estimated (lat, lon)
    std::pair<double, double> localize(const std::vector<RssiSample>& scan, int k = 3);
